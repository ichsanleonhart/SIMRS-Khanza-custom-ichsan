...
// --- METHOD BARU: UNTUK MEMECAH STRING ATURAN PAKAI MENJADI SIGNA 1 & 2 ---
    private double[] getSignaFromText(String textAturan) {
        double[] signa = {1, 1}; // Default 1 x 1 jika gagal parsing
        if (textAturan == null || textAturan.trim().equals("")) {
            return signa;
        }
        
        try {
            // Regex mencari pola: Angka (spasi) x/X (spasi) Angka
            // Contoh yang dikenali: "3x1", "3 x 1", "3 X 1", "2x0.5"
            java.util.regex.Pattern p = java.util.regex.Pattern.compile("(\\d+(?:\\.\\d+)?)\\s*[xX]\\s*(\\d+(?:\\.\\d+)?)");
            java.util.regex.Matcher m = p.matcher(textAturan);
            
            if (m.find()) {
                signa[0] = Double.parseDouble(m.group(1)); // Angka depan (Signa1)
                signa[1] = Double.parseDouble(m.group(2)); // Angka belakang (Signa2)
            }
        } catch (Exception e) {
            System.out.println("Gagal parsing signa aturan pakai: " + textAturan);
        }
        return signa;
    }
    ...
    public void tampilobat2(String no_resep) {     
        ...
        try {
            ...
            // 1. AMBIL DATA HEADER RESEP (Untuk mendapatkan No Rawat, Tgl, Jam)
            ps2 = koneksi.prepareStatement(
                "select resep_obat.no_resep, resep_obat.tgl_perawatan, resep_obat.jam, resep_obat.no_rawat, " +
                "pasien.no_rkm_medis, pasien.nm_pasien, resep_obat.kd_dokter, dokter.nm_dokter " +
                "from resep_obat " +
            ...
            try {
                ...
                while (rs2.next()) { 
                    String tglPerawatan = rs2.getString("tgl_perawatan");
                    String jamPerawatan = rs2.getString("jam");
                    String noRawat = rs2.getString("no_rawat");

                    // =================================================================================
                    // BAGIAN 1: OBAT NON RACIKAN (Join ke tabel aturan_pakai)
                    // =================================================================================
                    psobat = koneksi.prepareStatement(
                        "select databarang.kode_brng, databarang.nama_brng, detail_pemberian_obat.jml, " +
                        "ifnull(aturan_pakai.aturan,'') as aturan_teks " +
                        "from detail_pemberian_obat " +
                        "inner join maping_obat_apotek_bpjs on detail_pemberian_obat.kode_brng=maping_obat_apotek_bpjs.kode_brng " +
                        "inner join databarang on detail_pemberian_obat.kode_brng=databarang.kode_brng " +
                        // LEFT JOIN ke tabel aturan_pakai sesuai struktur user
                        "left join aturan_pakai on detail_pemberian_obat.tgl_perawatan=aturan_pakai.tgl_perawatan " +
                        ...
                        // Filter Data
                        "where detail_pemberian_obat.tgl_perawatan=? and detail_pemberian_obat.jam=? " +
                        "and detail_pemberian_obat.no_rawat=? " +
                        // Exclude obat yang masuk ke dalam racikan (cek tabel detail_obat_racikan)
                        "and databarang.kode_brng not in (" +
                        "   select detail_obat_racikan.kode_brng from detail_obat_racikan " +
                    ...
                    try {
                        psobat.setString(1, tglPerawatan);
                        psobat.setString(2, jamPerawatan);
                        psobat.setString(3, noRawat);
                        psobat.setString(4, tglPerawatan);
                        psobat.setString(5, jamPerawatan);
                        psobat.setString(6, noRawat);
                        rsobat = psobat.executeQuery();
                        
                        while(rsobat.next()){
                            // --- LOGIKA PARSING SIGNA ---
                            String teksAturan = rsobat.getString("aturan_teks");
                            double[] hasilSigna = getSignaFromText(teksAturan);
                            double signa1 = hasilSigna[0];
                            double signa2 = hasilSigna[1];
                            
                            // Hitung perkiraan hari
                            double jmlObat = rsobat.getDouble("jml");
                            double hari = 1;
                            if (signa1 > 0 && signa2 > 0) {
                                hari = Math.ceil(jmlObat / (signa1 * signa2));
                            }
                            // Default minimal 1 hari jika hasil 0
                            if (hari < 1) hari = 1;

                            // Masukkan ke tabel GUI
                            tabModeobat.addRow(new Object[] {
                                false,
                                rsobat.getString("jml"),
                                rsobat.getString("kode_brng"),
                                rsobat.getString("nama_brng"),
                                signa1, // Hasil Parsing
                                signa2, // Hasil Parsing
                                hari
                            }); 
                        }
                    } catch(Exception e) {
                        System.out.println("Notifikasi Non-Racikan : " + e);
                    } finally {
                    ...
                    // =================================================================================
                    // BAGIAN 2: OBAT RACIKAN (Menggunakan struktur obat_racikan & detail_obat_racikan)
                    // =================================================================================
                    psracikan = koneksi.prepareStatement(
                            ...
                            "obat_racikan.jml_dr, obat_racikan.aturan_pakai, " + // <-- Ini kolom aturan_pakai dari tabel obat_racikan
                            "obat_racikan.keterangan " +
                            "from obat_racikan " +
                            "inner join metode_racik on obat_racikan.kd_racik=metode_racik.kd_racik " +
                            "where obat_racikan.tgl_perawatan=? and obat_racikan.jam=? and obat_racikan.no_rawat=? ");
                    
                    try {
                        psracikan.setString(1, tglPerawatan);
                        psracikan.setString(2, jamPerawatan);
                        psracikan.setString(3, noRawat);
                        rsracikan = psracikan.executeQuery();
                        
                        while(rsracikan.next()){
                            // Simpan Data Master Racikan ke Tabel Atas
                            tabModeObatRacikan.addRow(new String[]{
                                rsracikan.getString("no_racik"),
                                rsracikan.getString("nama_racik"),
                                rsracikan.getString("kd_racik"),
                            ...
                            // Ambil Signa dari Master Racikan untuk diturunkan ke Detail
                            String aturanPakaiRacikan = rsracikan.getString("aturan_pakai");
                            double[] signaRacikan = getSignaFromText(aturanPakaiRacikan);
                            
                            // =========================================================
                            // BAGIAN 3: DETAIL RACIKAN (Isi Obat per Bungkus)
                            // =========================================================
                            psobat = koneksi.prepareStatement(
                                "select databarang.kode_brng, databarang.nama_brng, detail_pemberian_obat.jml " +
                                "from detail_pemberian_obat " +
                                ...
                                "where detail_pemberian_obat.tgl_perawatan=? and detail_pemberian_obat.jam=? " +
                                "and detail_pemberian_obat.no_rawat=? and detail_obat_racikan.no_racik=? " +
                                "order by databarang.kode_brng");
                            ...
                            try {
                                ...
                                while(rsobat.next()){
                                    // Ambil Kandungan dari tabel resep_dokter_racikan_detail
                                    // Sesuai struktur tabel ke-4 yang kamu berikan
                                    String kandungan = Sequel.cariIsi(
                                        "SELECT kandungan FROM resep_dokter_racikan_detail " +
                                        "WHERE no_resep='"+no_resep+"' " +
                                        "AND no_racik='"+rsracikan.getString("no_racik")+"' " +
                                        "AND kode_brng='"+rsobat.getString("kode_brng")+"'"
                                    );
                                    
                                    // Hitung Hari (Estimasi)
                                    double jmlObatDetail = rsobat.getDouble("jml");
                                    double hariRacikan = 1;
                                    if (signaRacikan[0] > 0 && signaRacikan[1] > 0) {
                                        // Untuk racikan, biasanya total bungkus yang dibagi, tapi disini kita estimasi dari bahan baku
                                        // Atau bisa pakai jml_dr dari master racikan (lebih akurat untuk racikan)
                                        try {
                                            double jmlDr = Double.parseDouble(rsracikan.getString("jml_dr"));
                                            hariRacikan = Math.ceil(jmlDr / (signaRacikan[0] * signaRacikan[1]));
                                        } catch (Exception e) {
                                            hariRacikan = 30; // Default fail-safe
                                        }
                                    }
                                    if (hariRacikan < 1) hariRacikan = 1;

                                    tabModeDetailObatRacikan.addRow(new Object[] {
                                        rsracikan.getString("no_racik"), // No Racik
                                        rsobat.getString("jml"),         // Jumlah Obat
                                        rsobat.getString("kode_brng"),   // Kode
                                        rsobat.getString("nama_brng"),   // Nama
                                        signaRacikan[0],                 // Signa 1 (Dari Master)
                                        signaRacikan[1],                 // Signa 2 (Dari Master)
                                        hariRacikan,                     // Hari
                                        kandungan                        // Kandungan
                                    });
                                }                                
                                ...
                                if(rsobat != null) rsobat.close();
                                if(psobat != null) psobat.close(); // Close prepared statement dalam loop
                            }
                        }
                    } catch (Exception e) {
                        System.out.println("Notif Master Racikan : " + e);
                    } finally {
                        if(rsracikan != null) rsracikan.close();
                        if(psracikan != null) psracikan.close();
                    }
