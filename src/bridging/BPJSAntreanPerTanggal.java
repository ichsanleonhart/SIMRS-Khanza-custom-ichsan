...
public final class BPJSAntreanPerTanggal extends javax.swing.JDialog {
    ...
    private String URL="",link="",utc="",requestJson="",datajam="",datajam2="",kodebooking="",data="",
              ...
              kodepoli="",kodedokter="",kodebpjs=Sequel.cariIsi("select password_asuransi.kd_pj from password_asuransi"), datataskid="";
...
//            Valid.pindah(evt, TCari, BtnAll);
    ...
    // 1. Ambil data booking dari tabel
    ...
    data = tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(), 0).toString(); // data = MJKN nobooking
    
    // 2. Tentukan Waktu Dasar (Base Time) untuk Rekonstruksi
    // Prioritas 1: Ambil dari jam_mulai di tabel jadwal
    String baseTimeQuery = "SELECT "+
        "    CONCAT(rp.tgl_registrasi, ' ', j.jam_mulai) "+
        "FROM "+
        "    reg_periksa rp "+
        "INNER JOIN "+
        "    jadwal j ON rp.kd_dokter = j.kd_dokter AND rp.kd_poli = j.kd_poli "+
        "WHERE "+
        "    rp.no_rawat = '"+kodebooking+"' AND j.hari_kerja = CASE "+ 
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Sunday' THEN 'AKHAD' "+
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Monday' THEN 'SENIN' "+
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Tuesday' THEN 'SELASA' "+
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Wednesday' THEN 'RABU' "+
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Thursday' THEN 'KAMIS' "+
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Friday' THEN 'JUMAT' "+
        "        WHEN DAYNAME(rp.tgl_registrasi) = 'Saturday' THEN 'SABTU' "+
        "        ELSE 'SENIN' "+ // Fallback jika setting hari berbeda
        "    END";
    String baseTime = Sequel.cariIsi(baseTimeQuery);
    
    // Prioritas 2 (Fallback): Ambil dari jam_reg di reg_periksa
    if (baseTime.equals("")) {
        baseTime = Sequel.cariIsi("SELECT CONCAT(tgl_registrasi, ' ', jam_reg) from reg_periksa where no_rawat=?", kodebooking);
    }

    // Jika Waktu Dasar tetap tidak ditemukan, batalkan proses.
    if (baseTime.equals("")) {
        System.out.println("Gagal menemukan waktu dasar (Base Time) for " + kodebooking + ". Proses rekonstruksi dibatalkan.");
        return; // Keluar dari fungsi
    }
    
    System.out.println("REKONSTRUKSI " + kodebooking + ". Waktu Dasar (Base Time): " + baseTime);

    // [MODIFIKASI] TAHAP 2.5: SINKRONISASI DAN HEALING SEBELUM PUSH
    System.out.println("Memulai proses Sync/Heal untuk MJKN nobooking: " + data);
    for (String taskId : new String[]{"3", "4", "5"}) {
        String waktuNyataBPJS = getWaktuTaskDariBPJS(data, taskId); // 'data' adalah MJKN nobooking
        
        if (waktuNyataBPJS != null && !waktuNyataBPJS.isEmpty()) {
            // BPJS Punya Data
            System.out.println("  -> [Sync API] BPJS memiliki Task " + taskId + " pada " + waktuNyataBPJS);
            boolean adaLokal = Sequel.cariIsi("select waktu from referensi_mobilejkn_bpjs_taskid where taskid=? and no_rawat=?", taskId, kodebooking).equals("");
            if (adaLokal) {
                // "Tambal" database lokal kita
                Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid", "?,?,?", "Patch Task " + taskId, 3, new String[]{kodebooking, taskId, waktuNyataBPJS});
                System.out.println("  -> HEALING SUKSES: No.Rawat " + kodebooking + " ditambal dengan waktu " + waktuNyataBPJS);
            } else {
                System.out.println("  -> [Sync] Data Task " + taskId + " lokal sudah sesuai.");
            }
        } else {
            // BPJS Tidak Punya Data
            System.out.println("  -> [Sync API] BPJS tidak memiliki data Task " + taskId + ".");
            String waktuLokal = Sequel.cariIsi("select waktu from referensi_mobilejkn_bpjs_taskid where taskid=? and no_rawat=?", taskId, kodebooking);
            if (!waktuLokal.equals("")) {
                // Hapus data "zombie" di lokal
                Sequel.queryu2("delete from referensi_mobilejkn_bpjs_taskid where taskid=? and no_rawat=?", taskId, kodebooking);
                System.out.println("  -> SINKRONISASI: Menghapus data 'zombie' Task " + taskId + " lokal (No.Rawat " + kodebooking + ").");
            }
        }
    }
    System.out.println("Proses Sync/Heal selesai.");

    // 3. Proses Rekonstruksi TASK ID 3 (Mulai Tunggu Poli)
    datataskid = Sequel.cariIsi("select * from referensi_mobilejkn_bpjs_taskid where taskid='3' and no_rawat=?", kodebooking);
    if (datataskid.equals("")) { // Hanya jika Task ID 3 bolong (Setelah di-heal)
        // Waktu Task 3 = Base Time + (60 s.d. 180 detik) -> (1-3 menit)
        datajam = Sequel.cariIsi("SELECT DATE_ADD('"+baseTime+"', INTERVAL (60 + (RAND() * 120)) SECOND)"); 
        
        if(!datajam.equals("")){
            // Simpan dulu ke tabel lokal agar tidak dobel kirim
            if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{kodebooking,"3",datajam}) == true){ 
                try {     
                    parsedDate = dateFormat.parse(datajam);
                    System.out.println("REKONSTRUKSI: Menjalankan WS taskid 3 (Mulai Tunggu Poli) pada " + datajam);
        ...
        System.out.println("REKONSTRUKSI: Task ID 3 sudah ada (setelah heal), dilewati.");
    }
    ...
    if (datataskid.equals("")) { // Hanya jika Task ID 4 bolong (Setelah di-heal)
        ...
        if(!datajam.equals("")){
            if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{kodebooking,"4",datajam}) == true){ 
                try {     
                    parsedDate = dateFormat.parse(datajam);
                    System.out.println("REKONSTRUKSI: Menjalankan WS taskid 4 (Mulai Pelayanan) pada " + datajam);
                    headers = new HttpHeaders();
                    headers.setContentType(MediaType.APPLICATION_JSON);
                    headers.add("x-cons-id",koneksiDB.CONSIDAPIMOBILEJKN());
                    utc=String.valueOf(api.GetUTCdatetimeAsString());
                    ...
                    System.out.println("Notifikasi Bridging Task 4 : "+ex);
                    Sequel.queryu2("delete from referensi_mobilejkn_bpjs_taskid where taskid='4' and no_rawat='"+kodebooking+"'"); 
                }
            }
        }
    } else {
        System.out.println("REKONSTRUKSI: Task ID 4 sudah ada (setelah heal), dilewati.");
    }
    ...
    if (datataskid.equals("")) { // Hanya jika Task ID 5 bolong (Setelah di-heal)
        ...
        if(!datajam.equals("")){
             if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{kodebooking,"5",datajam}) == true){ 
                try {     
                    parsedDate = dateFormat.parse(datajam);
                    System.out.println("REKONSTRUKSI: Menjalankan WS taskid 5 (Selesai Pelayanan) pada " + datajam);
                    headers = new HttpHeaders();
                    headers.setContentType(MediaType.APPLICATION_JSON);
                    headers.add("x-cons-id",koneksiDB.CONSIDAPIMOBILEJKN());
                    utc=String.valueOf(api.GetUTCdatetimeAsString());
                    ...
                    System.out.println("Notifikasi Bridging Task 5 : "+ex);
                    Sequel.queryu2("delete from referensi_mobilejkn_bpjs_taskid where taskid='5' and no_rawat='"+kodebooking+"'");
                }
             }
        }
    } else {
         System.out.println("REKONSTRUKSI: Task ID 5 sudah ada (setelah heal), dilewati.");
    }
    
    System.out.println("PROSES REKONSTRUKSI UNTUK " + kodebooking + " SELESAI.");
    ...
    // 1. Ambil kodebooking (no_rawat) dari tabel
    kodebooking = tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(), 0).toString(); // kodebooking = no_rawat
    ...
    System.out.println("REKONSTRUKSI NON-JKN " + kodebooking + ". Waktu Dasar (Base Time): " + baseTime);

    // [MODIFIKASI] TAHAP 2.5: SINKRONISASI DAN HEALING SEBELUM PUSH
    System.out.println("Memulai proses Sync/Heal untuk NON-JKN kodebooking: " + kodebooking);
    for (String taskId : new String[]{"3", "4", "5"}) {
        String waktuNyataBPJS = getWaktuTaskDariBPJS(kodebooking, taskId); // 'kodebooking' (no_rawat) digunakan sbg nobooking
        ...
        if(!datajam.equals("")){
            ...
            if(Sequel.menyimpantf2("referensi_mobilejkn_bpjs_taskid","?,?,?","task id",3,new String[]{kodebooking,"3",datajam}) == true){ 
                try {     
                    ...
                    headers.add("user_key",koneksiDB.USERKEYAPIMOBILEJKN());
                    requestJson ="{" +
                                     "\"kodebooking\": \""+kodebooking+"\"," + // Untuk Non-JKN, kodebooking adalah no_rawat
                                     "\"taskid\": \"3\"," +
                                     "\"waktu\": \""+parsedDate.getTime()+"\"" +
                                 "}";
                    System.out.println("JSON : "+requestJson+"\n");
    ...
    System.out.println("PROSES REKONSTRUKSI NON-JKN UNTUK " + kodebooking + " SELESAI.");
    ...
    */
    private String getWaktuTaskDariBPJS(String kodebooking, String taskIdTarget) {
        ...
        try {
            ...
            System.out.println("        -> [Sync API] Memanggil getlisttask untuk kodebooking: " + kodebooking);
            ...
            if (nameNode.path("code").asText().equals("200")) {
                // 2. Dekripsi respons
                JsonNode response = mapper.readTree(api.Decrypt(root.path("response").asText(), utcDekripsi)); 
                if (response.isArray()) {
                    // 3. Loop untuk mencari taskid 
                    for (JsonNode list : response) {
                        if (list.path("taskid").asText().equals(taskIdTarget)) {
                            ...
                            System.out.println("        -> [Sync API] Ditemukan " + list.path("taskname").asText() + " pada: " + waktuBPJS);
                            try {
                                ...
                                waktuRSTask = dateFormat.format(dateBPJS); // Hasil: "2025-10-31 10:00:00" (menggunakan dateFormat class)
                                
                                System.out.println("        -> [Sync API] Waktu dikonversi ke format MySQL: " + waktuRSTask);
                                
                            } catch (Exception e) {
                                System.out.println("        -> [Sync API] GAGAL PARSING WAKTU: " + e.getMessage());
                ...
                System.out.println("        -> [Sync API] Gagal mengambil list task: " + nameNode.path("message").asText());
            }
        } catch (Exception ex) {
            System.out.println("        -> [Sync API] Exception saat getlisttask: " + ex.getMessage());
