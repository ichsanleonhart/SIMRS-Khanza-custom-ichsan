...
<!-- DATA TAB -->
  ...
  <form method="GET" class="form-inline mb-3" onsubmit="return goToDataTab(this);">
    ...
    <div class="form-group mr-2">
      ...
      <input type="date" id="dari" name="dari" class="form-control" value="<?= $_GET['dari'] ?? '' ?>">
    </div>
    <div class="form-group mr-2">
      <label for="sampai" class="mr-2">Sampai</label>
      <input type="date" id="sampai" name="sampai" class="form-control" value="<?= $_GET['sampai'] ?? '' ?>">
    </div>
    <button type="submit" class="btn btn-primary mr-2">Filter</button>

    <?php if (isset($_GET['dari'], $_GET['sampai']) && $_GET['dari'] && $_GET['sampai']): ?>
      <a href="cetak_maintenance_rutin.php?dari=<?= $_GET['dari'] ?>&sampai=<?= $_GET['sampai'] ?>" target="_blank" class="btn btn-success">
