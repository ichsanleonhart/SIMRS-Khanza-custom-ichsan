...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['user_id'])) {
  ...
  // Hapus akses lama
  if (!mysqli_query($conn, "DELETE FROM akses_menu WHERE user_id = '$form_user_id'")) {
    die("Gagal hapus akses lama: " . mysqli_error($conn));
  }

  // Simpan akses baru
  foreach ($menu_ids as $menu_id) {
    $menu_id = (int) $menu_id;
    $insert_query = "INSERT INTO akses_menu (user_id, menu_id) VALUES ('$form_user_id', '$menu_id')";
    if (!mysqli_query($conn, $insert_query)) {
      die("Gagal simpan akses baru: " . mysqli_error($conn));
    }
  }
...
$total_pages = ceil($total_users / $limit);
                     ...
                     $queryAllUsers = mysqli_query($conn, "SELECT id, nik, nama, jabatan, unit_kerja FROM users ORDER BY nama ASC LIMIT $start, $limit");
                      ...
                      while ($row = mysqli_fetch_assoc($queryAllUsers)) :
                        ...
                        // Ambil menu yang sudah diakses oleh user ini
                      ...
                      <div class="modal fade" id="modalAkses<?= $row_user_id ?>" tabindex="-1" role="dialog" aria-labelledby="modalLabel<?= $row_user_id ?>" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                          <form method="post" class="modal-content" action="hak_akses.php">
  ...
  <!-- Daftar menu -->
  <div id="menu-list-<?= $row_user_id ?>">
    <?php foreach ($menus as $menu) : ?>
      <div class="form-check menu-item-<?= $row_user_id ?>">
        <input class="form-check-input" type="checkbox" name="menu[]" value="<?= $menu['id'] ?>" id="menu_<?= $row_user_id ?>_<?= $menu['id'] ?>" <?= in_array($menu['id'], $akses) ? 'checked' : '' ?>>
...
<script>
  // Checklist semua atau uncheck semua
