...
// 1. Security Check (Localhost Only)
...
// 2. Load Config Dynamic (Agar Tampilan Header Sesuai config.json)
...
if (file_exists($configFile)) {
    ...
    $display_host = $config['db_host'] ?? 'Unknown';
    $display_port = $config['node_port'] ?? 'Unknown';
}
?>
<!DOCTYPE html>
<html lang="id">
...
<div class="container">
    <div class="header">
        <div>
            ...
            <small>Database: <?php echo $display_host; ?> | Node: Port <?php echo $display_port; ?></small>
        </div>
        <div>
    ...
    <div id="log-window">
        <div class="log-line info">Sistem siap. Mode: Overworked Admin (Human Simulation).</div>
    </div>
</div>
...
<script>
    ...
    // AMAN: Karena appJM.js sudah punya delay "Ngetik" sendiri.
    // Delay ini hanya jeda "Bernapas" antara pesan sukses dan pesan berikutnya.
    const DELAY_MIN = 5; 
    const DELAY_MAX = 15;
    const DELAY_EMPTY = 5; 
    const AUTO_START_DELAY = 60; // 1 Menit cukup untuk boot Node.js

    // --- AUTO START LOGIC ---
    window.onload = function() {
        addLog(`⏳ Sistem akan AUTO-START dalam ${AUTO_START_DELAY} detik...`, 'warning');
