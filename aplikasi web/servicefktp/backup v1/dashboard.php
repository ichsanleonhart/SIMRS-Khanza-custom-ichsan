...
// File: dashboard.php
// Fungsi: Dashboard Antrean Online (Antrol) - 24/7 Stability Edition.
...
<head>
    ...
    <title>Service Antrol - Klinik Musytasyfah</title>
    ...
    <style>
        ...
        /* Indikator Detak Jantung Aplikasi */
        @keyframes pulse-dot {
            0% { opacity: 1; transform: scale(1); box-shadow: 0 0 0 0 rgba(59, 130, 246, 0.7); }
            70% { opacity: 1; transform: scale(1); box-shadow: 0 0 0 10px rgba(59, 130, 246, 0); }
...
<body class="bg-gray-900 text-gray-200 min-h-screen font-sans p-6 flex flex-col">
    ...
    <div class="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
        <div class="flex items-center gap-3">
            <div class="w-3 h-3 bg-blue-500 rounded-full live-dot"></div>
            <div>
                <h1 class="text-2xl font-bold text-white tracking-wide">
                    SERVICE <span class="text-blue-500">ANTROL BPJS</span>
                </h1>
                <p class="text-xs text-gray-400">Worker Otomatis (Task 0, 1, 2)</p>
            </div>
        </div>
        ...
        <div class="flex gap-4 items-center">
            <div class="text-right">
                ...
                <div id="clock" class="text-xl font-mono font-bold text-blue-400">00:00:00</div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-gray-800 rounded-lg p-5 border border-gray-700 shadow-lg">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="bg-cyan-900 text-cyan-200 px-2 py-0.5 rounded text-xs font-mono">BPJS Only</span>
                </div>
            ...
            <div class="bg-gray-800 rounded-lg p-5 border border-gray-700 shadow-lg">
                <h2 class="text-sm font-bold text-gray-400 mb-4 uppercase">Resend (Manual)</h2>
                
                <div class="space-y-3">
                    <div>
                        <label class="text-[10px] text-gray-500">Dari Tanggal</label>
                        <input type="date" id="tgl_mulai" class="w-full bg-gray-900 text-white border border-gray-600 rounded px-2 py-1.5 text-sm focus:border-blue-500 outline-none">
                    </div>
                    <div>
                        <label class="text-[10px] text-gray-500">Sampai Tanggal</label>
                        <input type="date" id="tgl_akhir" class="w-full bg-gray-900 text-white border border-gray-600 rounded px-2 py-1.5 text-sm focus:border-blue-500 outline-none">
                    </div>
                    
                    <button onclick="runResend()" id="btn-sapu" class="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded text-sm transition shadow-lg mt-2">
                        🔁 Kirim Ulang
                    </button>
                </div>
            </div>
        ...
        <div class="lg:col-span-3 flex flex-col bg-black rounded-lg border border-gray-700 shadow-2xl overflow-hidden h-[600px] lg:h-auto">
            <div class="bg-gray-900 px-4 py-2 flex justify-between items-center border-b border-gray-800">
                <span class="text-xs font-mono text-blue-500">root@antrol-service:~/logs$ tail -f activity.log</span>
            ...
            <div id="log-container" class="flex-1 p-4 overflow-y-auto terminal-log text-xs space-y-1 scroll-smooth">
                <div class="text-gray-500 italic">System initialized. Antrol Worker ready...</div>
            </div>
        </div>
    </div>
    ...
    <script>
        let isRunning = true;
        let workerTimeout;
        const loadedDate = new Date().getDate(); // Simpan tanggal saat load untuk referensi Refresh
        ...
        setInterval(() => {
            ...
            // [FITUR 24 JAM] Auto Refresh saat ganti hari (00:00)
            // Ini mencegah dashboard 'stuck' di tanggal kemarin
        ...
        function addLog(msg, type = 'info') {
            ...
            else if (type === 'system') color = 'text-blue-400 font-bold';
            ...
            // Ini mencegah browser crash karena kehabisan memori DOM
        ...
        function startWorkerCycle() {
            ...
            $.ajax({
                url: 'sync_worker.php', // Worker Antrol
                method: 'GET',
                dataType: 'json',
                timeout: 30000, 
                success: function(res) {
                    if (res.status === 'success') {
                        ...
                        if (res.logs && res.logs.length > 0) {
                            res.logs.forEach(l => {
                                const type = (l.includes('GAGAL')) ? 'error' : 'success';
                                addLog(l, type);
                            });
                        }
                    } else {
                ...
                complete: function() {
                    // [FITUR NON-BLOCKING]
                    // Gunakan setTimeout rekursif, bukan setInterval.
                    // Ini memastikan request berikutnya baru jalan SETELAH request sebelumnya selesai.
                    // Mencegah "Traffic Pile-up" jika server sedang lambat.
                    if (isRunning) {
        ...
        $('#toggle-service').change(function() {
            if(this.checked) {
                ...
                $('.dot').addClass('transform translate-x-full bg-blue-400').removeClass('bg-white');
                ...
                $('.dot').removeClass('transform translate-x-full bg-blue-400').addClass('bg-white');
        ...
        async function runResend() {
            ...
            addLog(`=== START RESEND ${tgl1} s.d ${tgl2} ===`, 'system');
            ...
            addLog("=== SELESAI ===", 'system');
            $('#btn-sapu').prop('disabled', false).text('🔁 Kirim Ulang');
        }

        function processDate(tgl) {
            return new Promise((resolve) => {
                $.ajax({
                    url: 'resend_worker.php', // Worker Manual Antrol
                    method: 'POST',
                    data: { tgl_mulai: tgl, tgl_akhir: tgl },
                    dataType: 'json',
                    success: function(res) {
                        if(res.status === 'success') {
                            if(res.logs && res.logs.length > 0) {
                    ...
                    error: function() {
                        ...
                        resolve(); // Tetap lanjut tgl berikutnya meski error
                    }
                });
            });
        }
        ...
        $(document).ready(function() {
            ...
            // Start automatically
            addLog("System initialized. Starting Antrol worker...", 'info');
            startWorkerCycle();
        });
    </script>
</body>
