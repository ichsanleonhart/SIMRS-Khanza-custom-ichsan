...
try {
    ...
    $r_jam = fetchOneSafe($pdo, "SELECT hariawal, lamajam FROM set_jam_minimal LIMIT 1");
    if($r_jam) $setting_kamar = $r_jam;

    // 2. INFO PASIEN
    $pasien = fetchOneSafe($pdo, "SELECT p.nm_pasien, p.no_rkm_medis, rp.status_lanjut, rp.kd_pj, ki.tgl_masuk, ki.jam_masuk, b.nm_bangsal, pj.png_jawab 
        FROM reg_periksa rp 
    ...
    // 3. SETTING PPN
    $pakai_ppn = false;
    $r_set = fetchOneSafe($pdo, "SELECT tampilkan_ppnobat_ralan, tampilkan_ppnobat_ranap FROM set_nota LIMIT 1");
    if($r_set) {
    ...
    // A. REGISTRASI
    $reg = fetchOneSafe($pdo, "SELECT rp.biaya_reg, b.nm_bangsal FROM reg_periksa rp LEFT JOIN kamar_inap ki ON rp.no_rawat = ki.no_rawat LEFT JOIN kamar k ON ki.kd_kamar = k.kd_kamar LEFT JOIN bangsal b ON k.kd_bangsal = b.kd_bangsal WHERE rp.no_rawat = ?", [$no_rawat]);
    if ($reg) {
    ...
    $drs = fetchAllSafe($pdo, "SELECT d.nm_dokter FROM rawat_inap_dr rid JOIN dokter d ON rid.kd_dokter = d.kd_dokter WHERE rid.no_rawat=? GROUP BY rid.kd_dokter UNION SELECT d.nm_dokter FROM rawat_jl_dr rjd JOIN dokter d ON rjd.kd_dokter = d.kd_dokter WHERE rjd.no_rawat=? GROUP BY rjd.kd_dokter", [$no_rawat, $no_rawat]);
    foreach($drs as $dr) {
        $rincian[] = ['kategori' => '', 'nama' => $dr['nm_dokter'], 'biaya' => 0, 'header' => true];
    }

    // C. KAMAR INAP
    $kamars = fetchAllSafe($pdo, "SELECT k.kd_kamar, b.nm_bangsal, k.trf_kamar, ki.tgl_masuk, ki.jam_masuk, ki.tgl_keluar, ki.jam_keluar, ki.lama, ki.ttl_biaya FROM kamar_inap ki JOIN kamar k ON ki.kd_kamar = k.kd_kamar JOIN bangsal b ON k.kd_bangsal = b.kd_bangsal WHERE ki.no_rawat = ?", [$no_rawat]);
    foreach($kamars as $r) {
        ...
        // Biaya Sekali & Harian
        $kd = $r['kd_kamar'];
        $biaya_sekali = fetchAllSafe($pdo, "SELECT nama_biaya, besar_biaya FROM biaya_sekali WHERE kd_kamar=?", [$kd]);
        ...
        $biaya_harian = fetchAllSafe($pdo, "SELECT nama_biaya, besar_biaya FROM biaya_harian WHERE kd_kamar=?", [$kd]);
    ...
    $ol = fetchOneSafe($pdo, "SELECT besar_tagihan FROM tagihan_obat_langsung WHERE no_rawat=?", [$no_rawat]);
    if ($ol) {
    ...
    // 2. Beri Obat Operasi
    $oop = fetchAllSafe($pdo, "SELECT o.nm_obat, b.hargasatuan, b.jumlah, (b.hargasatuan * b.jumlah) as total FROM beri_obat_operasi b JOIN obatbhp_ok o ON b.kd_obat = o.kd_obat WHERE b.no_rawat=?", [$no_rawat]);
    foreach($oop as $r) {
    ...
    $dpo = fetchAllSafe($pdo, "SELECT d.nama_brng, dp.total FROM detail_pemberian_obat dp JOIN databarang d ON dp.kode_brng = d.kode_brng WHERE dp.no_rawat=?", [$no_rawat]);
    foreach($dpo as $r) {
        $val = safeFloat($r['total']);
    ...
    // Retur Obat
    $returs = fetchAllSafe($pdo, "SELECT d.nama_brng, r.jml, (r.jml * d.ralan) as total_estimasi FROM returpasien r JOIN databarang d ON r.kode_brng = d.kode_brng WHERE r.no_rawat=?", [$no_rawat]);
    foreach($returs as $r) {
    ...
    $tindakan = fetchAllSafe($pdo, $sql_tind, $params_tind);
    ...
    $ops = fetchAllSafe($pdo, "SELECT p.nm_perawatan, o.* FROM operasi o JOIN paket_operasi p ON o.kode_paket = p.kode_paket WHERE o.no_rawat=?", [$no_rawat]);
    foreach($ops as $r) {
    ...
    // G. TAMBAHAN & POTONGAN
    $adds = fetchAllSafe($pdo, "SELECT nama_biaya, besar_biaya FROM tambahan_biaya WHERE no_rawat=?", [$no_rawat]);
    foreach($adds as $r) {
    ...
    $mins = fetchAllSafe($pdo, "SELECT nama_pengurangan, besar_pengurangan FROM pengurangan_biaya WHERE no_rawat=?", [$no_rawat]);
    foreach($mins as $r) {
    ...
    if ($status_lanjut == 'Ranap') {
        ...
        $s = fetchOneSafe($pdo, "SELECT * FROM $tabel_service LIMIT 1");
        ...
        if ($s) {
            ...
            if ($total_basis > 0 && $persen > 0) {
                ...
                $cek = fetchOneSafe($pdo, "SELECT totalbiaya FROM billing WHERE no_rawat=? AND (nm_perawatan LIKE '%Administrasi%' OR nm_perawatan LIKE '%Service%')", [$no_rawat]);
                if (!$cek) {
