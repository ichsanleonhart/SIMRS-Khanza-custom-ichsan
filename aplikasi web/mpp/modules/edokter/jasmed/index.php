<?php
// File: modules/edokter/jasmed/index.php
...
try {
    ...
    // [FIX 1970 BUG]: Menambahkan ALIAS EKSPLISIT ('as nama_kolom') di setiap query agar PHP mengenalinya!
...
$export_params = http_build_query([
    'tgl_awal' => $tgl_awal, 'tgl_akhir' => $tgl_akhir, 'kd_dokter' => $kd_dokter, 'kategori' => $kategori
]);
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="h5 mb-0 text-gray-800"><i class="fas fa-wallet text-warning me-2"></i> Audit Jasa Medis</h4>
        <div>
            <a href="ringkasan_shift.php?<?= $export_params ?>" class="btn btn-primary btn-sm"><i class="fas fa-clock"></i> Ringkasan Per Shift</a>
        </div>
    </div>
        ...
        <div class="card-body py-2">
            <form method="GET" action="">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-4">
                        <label class="small fw-bold">Pilih Dokter (Mode Admin)</label>
                    ...
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-success btn-sm w-100"><i class="fas fa-search"></i> Tampilkan</button>
                    </div>
                </div>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-white py-2 d-flex justify-content-between align-items-center">
            <span class="fw-bold text-success">Rincian Tindakan & Jasmed</span>
            <a href="export.php?<?= $export_params ?>" class="btn btn-success btn-sm"><i class="fas fa-file-excel"></i> Download Excel</a>
        </div>
        <div class="card-body p-3 table-responsive">
            <table class="table table-hover table-bordered mb-0 align-middle w-100" id="tblJasmed">
                <thead class="bg-light text-center small">
                    <tr>
                        ...
                        <th width="12%">Biaya Pasien (Rp)</th>
                        <th width="13%">JM Dokter (Rp)</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    ...
                    foreach($hasil as $row): 
                        // Jika ternyata masih ada baris kosong akibat data rusak di database lama, kita skip
                    ...
                    ?>
                    <tr>
                        <td class="text-center" data-sort="<?= strtotime($row['tgl_transaksi']) ?>"><small><?= date('d/m/Y H:i', strtotime($row['tgl_transaksi'])) ?></small></td>
                        <td class="text-center"><?= htmlspecialchars($row['no_rkm_medis']) ?></td>
                        <td><?= htmlspecialchars($row['nm_pasien']) ?><br><small class="text-muted"><?= htmlspecialchars($row['no_rawat']) ?></small></td>
                        <td><?= htmlspecialchars($row['tindakan']) ?></td>
                        <td class="text-center"><span class="badge bg-secondary"><?= htmlspecialchars($row['sumber']) ?></span></td>
                        <td class="text-end <?= (!$add_cost && $row['total_biaya']>0) ? 'text-muted text-decoration-line-through' : 'fw-bold' ?>">
                            <?= number_format($row['total_biaya'], 0, ',', '.') ?>
                        </td>
                        <td class="text-end fw-bold text-success"><?= number_format($row['jm_dokter'], 0, ',', '.') ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
                <tfoot class="bg-light">
                    <tr class="fw-bold">
                        <td colspan="5" class="text-end">TOTAL BERSIH (Tanpa Duplikasi Biaya):</td>
                        <td class="text-end text-danger">Rp <?= number_format($sum_biaya, 0, ',', '.') ?></td>
                        <td class="text-end text-success">Rp <?= number_format($sum_jm, 0, ',', '.') ?></td>
                    </tr>
                </tfoot>
            </table>
        </div>
...
$(document).ready(function() {
    ...
    // Inisialisasi DataTables Universal (Sort, Search, Paging)
    $('#tblJasmed').DataTable({
        "ordering": true,
        "order": [[ 0, "asc" ]], // Sort otomatis berdasarkan kolom waktu (indeks 0)
        "language": { 
            "search": "Pencarian Universal:",
            "lengthMenu": "Tampilkan _MENU_ data per halaman",
            "zeroRecords": "Tidak ada data transaksi jasmed ditemukan.",
