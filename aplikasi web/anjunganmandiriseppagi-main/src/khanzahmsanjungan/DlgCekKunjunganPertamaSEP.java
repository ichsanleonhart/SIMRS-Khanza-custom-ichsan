...
public class DlgCekKunjunganPertamaSEP extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        btnCekRujukanRS = new component.Button();
        ...
        TanggalRujukKeluar.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "09-06-2025" }));
        ...
        TanggalKunjungRujukan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "09-06-2025" }));
        ...
        WindowMenu.getContentPane().setLayout(new java.awt.GridLayout(1, 0));
        ...
        btnCekRujukanRS.setForeground(new java.awt.Color(0, 131, 62));
        btnCekRujukanRS.setMnemonic('K');
        btnCekRujukanRS.setText("CEK RUJUKAN RS");
        btnCekRujukanRS.setToolTipText("Alt+K");
        btnCekRujukanRS.setFont(new java.awt.Font("Poppins", 0, 18)); // NOI18N
        btnCekRujukanRS.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        btnCekRujukanRS.setPreferredSize(new java.awt.Dimension(250, 45));
        btnCekRujukanRS.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnCekRujukanRSActionPerformed(evt);
            }
        });
        btnCekRujukanRS.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                btnCekRujukanRSKeyPressed(evt);
            }
        });
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 5;
        gridBagConstraints.gridy = 13;
        jPanel1.add(btnCekRujukanRS, gridBagConstraints);
...
//                Notifications.getInstance().show(Notifications.Type.WARNING, Notifications.Location.TOP_CENTER, "Data surat kontrol tidak ditemukan!");
    ...
    private void btnCekRujukanRSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCekRujukanRSActionPerformed
        if (!NoRMPasien.getText().equals("")) {
            if (Sequel.cariInteger("select count(referensi_mobilejkn_bpjs.norm) from referensi_mobilejkn_bpjs where referensi_mobilejkn_bpjs.norm='" + NoRMPasien.getText() + "' and referensi_mobilejkn_bpjs.tanggalperiksa=current_date") > 0) {
                JOptionPane.showMessageDialog(rootPane, "Pasien telah menggunakan Mobile JKN. Silahkan cekin menggunakan menu MobileJKN");
            } else if (Sequel.cariInteger("select count(referensi_mobilejkn_bpjs.nomorkartu) from referensi_mobilejkn_bpjs where referensi_mobilejkn_bpjs.nomorkartu='" + NoRMPasien.getText() + "' and referensi_mobilejkn_bpjs.tanggalperiksa=current_date") > 0) {
                ...
                if (Sequel.cariInteger("select count(pasien.no_peserta) from pasien where pasien.no_peserta='" + NoRMPasien.getText() + "'") == 1) {
                    this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                    DlgRegistrasiSEPPertama form = new DlgRegistrasiSEPPertama(null, true);
                    form.tampilRujukanRS(NoRMPasien.getText());
                 //   form.tampilfingerprint(NoRMPasien.getText());
                    form.setSize(this.getWidth(), this.getHeight());
                    form.setLocationRelativeTo(jPanel1);
                    this.dispose();
                    ...
                    this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                    DlgRegistrasiSEPPertama form = new DlgRegistrasiSEPPertama(null, true);
                    form.tampilRujukanRS(Sequel.cariIsi("select pasien.no_peserta from pasien where pasien.no_rkm_medis='" + NoRMPasien.getText() + "'"));
                //    form.tampilfingerprint(Sequel.cariIsi("select pasien.no_peserta from pasien where pasien.no_rkm_medis='" + NoRMPasien.getText() + "'"));
                    form.setSize(this.getWidth(), this.getHeight());
                    form.setLocationRelativeTo(jPanel1);
                    this.dispose();
                    ...
                    this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                    DlgRegistrasiSEPPertama form = new DlgRegistrasiSEPPertama(null, true);
                    form.tampilRujukanRS(Sequel.cariIsi("select pasien.no_peserta from pasien where pasien.no_ktp='" + NoRMPasien.getText() + "'"));
                  //  form.tampilfingerprint(Sequel.cariIsi("select pasien.no_peserta from pasien where pasien.no_ktp='" + NoRMPasien.getText() + "'"));
                    form.setSize(this.getWidth(), this.getHeight());
                    form.setLocationRelativeTo(jPanel1);
                    this.dispose();
    ...
    }//GEN-LAST:event_btnCekRujukanRSActionPerformed

    private void btnCekRujukanRSKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_btnCekRujukanRSKeyPressed
        
    }//GEN-LAST:event_btnCekRujukanRSKeyPressed
    ...
    private component.Button btnCekRujukanRS;
