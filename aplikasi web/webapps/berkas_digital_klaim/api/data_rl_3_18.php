 ...
 * Fungsi: Laporan RL 3.18 dengan Grouping Dinamis (Golongan/Kategori/Jenis)
 */
...
$mode       = $_GET['mode'] ?? 'golongan'; // Default grouping

// 2. TENTUKAN KOLOM & JOIN BERDASARKAN MODE
$select_group = "";
$join_group   = "";
$group_by     = "";
$col_id       = ""; // ID untuk dikirim ke detail

switch ($mode) {
    case 'jenis':
        $select_group = "COALESCE(j.nama, 'Tanpa Jenis') as nama_group";
        $col_id       = "db.kdjns as kode_group"; // ID Asli
...
$sql = "
    SELECT 
        ...
        -- PIVOT DATA
        SUM(IF(rp.status_lanjut = 'Ralan' AND rp.kd_poli != 'IGDK', dpo.jml, 0)) as jml_ralan,
        SUM(IF(rp.kd_poli = 'IGDK', dpo.jml, 0)) as jml_igd,
        SUM(IF(rp.status_lanjut = 'Ranap', dpo.jml, 0)) as jml_ranap,
        SUM(dpo.jml) as total_semua
...
if ($stmt) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        // Casting ke float agar JSON number, bukan string
        ...
        // Handle null kode_group (jika data master kosong)
