                        ...
                        <table id="tablePasien" class="table table-striped table-hover mb-0 w-100">
                            ...
                            <tbody>
                                ...
                                if($hasil){ while ($row = mysqli_fetch_assoc($hasil)) { $is_igd = ($row['kd_poli'] == 'IGDK'); ?>
                                    <tr>
                                        <td><span class="fw-bold d-block"><?= $row['no_rawat'] ?></span><small class="text-muted"><?= $row['tgl_closing'] ?></small></td>
                                        <td><div class="fw-bold text-truncate" style="max-width:150px;"><?= $row['nm_pasien'] ?></div><small class="text-primary text-truncate d-block" style="max-width:150px;"><?= $row['nm_dokter'] ?></small></td>
                                        <td><span class="badge bg-light text-dark border d-block mb-1"><?= $row['no_sep'] ?></span><span class="badge <?= ($row['status_lanjut']=='Ralan')?'bg-success':'bg-pink' ?>"><?= $row['status_lanjut'] ?></span></td>
...
<script>
    ...
    $(document).ready(function() {
        $('#tablePasien').DataTable({
            dom: 'Bfrtip', pageLength: 15, ordering: false,
